...
namespace Components {
	Mover::Mover(const Cori::Physics::Capsule& moverCapsule, const Cori::Physics::Capsule& sensorCapsule, Cori::Physics::WorldRef world, Cori::World::Entity& player, const Params& def) : // GOD FUCKING LORD THATS A HUGE INIT-LIST
		...
		m_WallJumpVariableSideSpeed(def.wallJumpVariableSideSpeed), m_WallJumpVariableTicks(def.wallJumpVariableTicks), m_WallJumpBufferTicks(def.wallJumpBufferTicks),
			m_DoubleJumpStartSpeed(def.doubleJumpStartSpeed), m_DoubleJumpMaxStartSpeed(def.doubleJumpMaxStartSpeed), m_DoubleJumpVariableSpeed(def.doubleJumpVariableSpeed), m_DoubleJumpVariableTicks(def.doubleJumpVariableTicks),
	...
	void Mover::OnUpdate(const double deltaTime, const double tickAlpha) {
		...
		//Cori::Graphics::Renderer2D::SubmitColoredQuad(Cori::Graphics::Renderer2D::WORLD_SPACE, Cori::Physics::ToPixels(target), glm::vec2(1.0f, 1.0f), glm::vec3(1.0f, 0.0f, 1.0f));
		//
	...
	void Mover::OnTickUpdate(const float timeStep, MainCamera& mainCamera, bool playerResponsive) {
		...
		// vvv double jump raycast checks, to avoid double jumping when player is almoust touching the ground or the wall
		{
			...
			m_GroundRayEnd = { m_GroundRayStart.x, m_GroundRayStart.y + (m_RelativeVelocity.y * timeStep * static_cast<float>(m_JumpBufferTicks) / m_DoubleJumpRayModifierForRegular) };
			{
				...
				if (rayTranslation.y < 0.0f) {
					rayResult = m_World.CastRayClosest(m_GroundRayStart, rayTranslation, colliderFilter);
				}
...

			{
				...
				rayResult = m_World.CastRayClosest(m_WallRayStart, rayTranslation, colliderFilter);
	...
	void Mover::AddStunTicks(const uint32_t ticks) {
		m_StunTicksLeft += ticks;
	}
	...
	void Mover::UpdateGui() {
		...
		ImGui::SliderFloat("Double Jump Max Start Speed", &m_DoubleJumpMaxStartSpeed, 0.0f, 120.0f, "%.1f");
...
//
	...
	void Mover::SolveMove(const float timeStep, float throttle, const Cori::Physics::Vec2 naturalVelocity) {
		...
		{
			m_PogoCurrentLength = m_CastResult.fraction * rayLength;

			float offset = m_PogoCurrentLength - pogoRestLength;
