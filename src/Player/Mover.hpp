...
namespace Components {
	...
	public:
		...
		struct Params {
			...
			float doubleJumpMaxStartSpeed{ 15.0f };
		...
		void AddImpulse(const Cori::Physics::Vec2 impulse, const Cori::Physics::Vec2 currentVelocityModifier);

		void AddImpulse(const Cori::Physics::Vec2 impulse);

		void AddStunTicks(const uint32_t ticks);

		void UpdateGui();
		void DebugDraw(const float timeStep);
		...
		float m_DoubleJumpMaxStartSpeed;
