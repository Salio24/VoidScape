...
namespace Systems {
	...
	void MovingPlatform::OnTickUpdate(Cori::Core::GameTimer& gameTimer) {
		...
		for (const auto entity : view) {
			...
			transform.p = {
				static_cast<float>(pc.m_ExpressionX(m_InternalTimer)) + pc.m_PositionOffset.x,
				static_cast<float>(pc.m_ExpressionY(m_InternalTimer)) + pc.m_PositionOffset.y
			};
		...
		m_InternalTimer += gameTimer.GetTimestep();
	}
	...
	void MovingPlatform::CreatePlatform(PlatformParams& params) {
		auto system = m_Owner.GetSystem<Cori::World::Systems::PhysicsSystem>();
		if (system) {
			...
			auto& rb = platform.AddComponent<Cori::World::Components::Entity::RigidBody>(locked->GetWorld(), bp);
			...
			CORI_FATAL("Failed to create moving platform, no physics system registered for the scene.");
		}
